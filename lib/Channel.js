...
function channel(type, arg, callback)
{
  ...
  chan.receive = function(packet)
  {
    ...
    if(!chan.acker)
      chan.acker = setTimeout(function(){ delete chan.acker; chan.ack();}, defaults.chan_autoack);
  ...
  chan.handler = function()
  {
    ...
    chan.callback(chan.ended, packet, chan, function(ack){
      ...
      chan.handling = false;
      if(ack)
        chan.ack(); // auto-ack functionality
      // cleanup eventually
      if(chan.ended)
        cleanup();
      chan.handler();
    ...
    if(arg.retry)
    {
      ...
      function retry(){
        if(chan.ended || chan.opened)
          return; // means we're gone or received a packet
        chan.send(arg);
        if(at < 4000)
          at *= 2;
        arg.retry--;
