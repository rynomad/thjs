  ...
  , Switch = function Switch()
{
  var self = {
    ...
    , online : function online(callback)
      {
        ...
        self.isOnline = true;
        // ping lan
        self.send({type:"lan"}, PacketFn.encode(self.ping()));
    ...
    , raws : ins
    , seek : seek
    , pencode : PacketFn.encode
    , pdecode : PacketFn.decode
    , isLocalIP : etc.isLocalIP
    , randomHEX : etc.randomHEX
    , uriparse  : function uriparse(uri)
      {
        ...
        if(u.hostname != part)
          return urllib.parse(uri); // hashname was not the hostname
    ...
    , isHashname : function(hex){return isHEX(hex, 64)}
    , isBridge   : function isBridge(arg)
      {
        var self = this;
